...
- 「System」をクリックするとメモリ内訳と、CPU・メモリの上位 5 アプリを展開（プロセス一覧は開いている間だけ取得します）
- 歯車メニューの「表示項目」で出す項目を選択。外した項目は取得もサンプリングもしないので、使わないサービスの負荷はゼロ
- 歯車メニューから「ログイン時に起動」を設定
- 歯車メニューの「使用量が多くなったら通知」を入れると、各枠（5 時間・週・月など）の使用量が 80% と 95% を超えたときに通知します。同じ枠で同じ段階は 1 度だけで、通知をクリックするとパネルが開きます。有効な間はパネルを閉じていても 10 分ごとに取得します
- 「毎朝、週・月の枠の残りを通知」を入れると、1 日 1 回（9 時を過ぎてから最初の取得時）に週・月の枠の残りとリセットまでの時間を、残りの少ない順に 1 件にまとめて通知します（5 時間枠は載せません）
...
名前は実行ファイル名ではなくアプリ名で出し、同じアプリのヘルパープロセス（Electron 系の Renderer / GPU など）は
1 行にまとめて合計します（まとめた件数は `×3` のように添えます）。
