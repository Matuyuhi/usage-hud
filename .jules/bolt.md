...
## 2026-09-25 - Avoid unnecessary string allocations in replacingOccurrences

**Learning:** In Swift, calling `replacingOccurrences(of:with:)` on a `Substring` implicitly allocates a new `String` object, even if the target character is absent. In tight polling loops processing many lines of text, this can cause unnecessary memory churn and CPU overhead.
**Action:** For simple string replacements in tight loops, avoid unnecessary allocations by wrapping it in a `.contains()` check so that the string allocation and replacement only happens when the replacement is actually needed.
