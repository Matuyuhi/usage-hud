...
## 2024-11-20 - Avoid String allocations when replacing occurrences in Substring

**Learning:** In tight parsing loops (like parsing ps output for hundreds of processes), calling `replacingOccurrences(of:with:)` directly on a `Substring` implicitly forces a conversion to a new `String` object, even if the substring doesn't contain the target character. This causes thousands of unnecessary allocations.
**Action:** Before applying replacement functions that implicitly allocate, use `.contains()` to check if the replacement is actually needed, keeping the fast path allocation-free.
