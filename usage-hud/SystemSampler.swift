...
final class SystemSampler {
    ...
    private let kernelPageSize: vm_size_t

    init() {
        self.hostPort = mach_host_self()
        var pageSize: vm_size_t = 0
        host_page_size(self.hostPort, &pageSize)
        self.kernelPageSize = pageSize
    }

    deinit {
        mach_port_deallocate(mach_task_self_, self.hostPort)
    }
    ...
    private func memoryBreakdown() -> (active: UInt64, wired: UInt64, compressed: UInt64) {
        ...
        // グローバル変数の vm_kernel_page_size は並行アクセス安全でないためキャッシュ版を使う
