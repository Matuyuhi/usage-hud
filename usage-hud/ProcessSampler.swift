...
nonisolated enum AppNames {
    ...
    static func resolve(executablePath path: String) -> Resolved {
        guard let bundlePath = outermostAppBundle(in: path) else {
            // Optimize: avoid NSString bridging in tight loop. Use native String operations.
            let lastComponent = path.lastIndex(of: "/").map { String(path[path.index(after: $0)...]) } ?? path
            return Resolved(key: path, name: lastComponent)
        }
...
private nonisolated final class BundleNameCache: @unchecked Sendable {
    ...
    private static func read(_ bundlePath: String) -> String {
        // Optimize: avoid NSString bridging (lastPathComponent, deletingPathExtension) in tight loop
        let lastComponent = bundlePath.lastIndex(of: "/").map { String(bundlePath[bundlePath.index(after: $0)...]) } ?? bundlePath
        let fallback = lastComponent.lastIndex(of: ".").map { String(lastComponent[..<$0]) } ?? lastComponent
