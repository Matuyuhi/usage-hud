...
nonisolated enum AppNames {
    ...
    static func resolve(executablePath path: String) -> Resolved {
        guard let bundlePath = outermostAppBundle(in: path) else {
            // Optimize: avoid NSString bridging in polling loop
...
private nonisolated final class BundleNameCache: @unchecked Sendable {
    ...
    private static func read(_ bundlePath: String) -> String {
        // Optimize: avoid NSString bridging
        let nameWithExt = bundlePath.lastIndex(of: "/").map { String(bundlePath[bundlePath.index(after: $0)...]) } ?? bundlePath
        let fallback = nameWithExt.hasSuffix(".app") ? String(nameWithExt.dropLast(4)) : nameWithExt
